...
#undef  x264_8_macroblock_write_cavlc
#define x264_8_macroblock_write_cavlc  static macroblock_8_size_cavlc
#include "cavlc-8.c"
...
#undef  x264_8_macroblock_write_cabac
#define x264_8_macroblock_write_cabac  static macroblock_8_size_cabac
#include "cabac-8.c"
...
static ALWAYS_INLINE uint64_t cached_8_hadamard( x264_t *h, int size, int x, int y )
{
...
static ALWAYS_INLINE int cached_8_satd( x264_t *h, int size, int x, int y )
{
...
static inline int ssd_8_plane( x264_t *h, int size, int p, int x, int y )
{
    int satd = 0;
    ...
    if( p == 0 && h->mb.i_psy_rd )
    {
        ...
        if( size <= PIXEL_8x8 )
        {
            ...
            uint64_t fenc_acs = cached_8_hadamard( h, size, x, y );
        ...
        else
        {
            ...
            satd = abs(h->pixf.satd[size]( fdec, FDEC_STRIDE, (pixel*)x264_zero, 0 ) - dc - cached_8_satd( h, size, x, y ));
        }
...
static inline int ssd_8_mb( x264_t *h )
{
    int i_ssd = ssd_8_plane( h, PIXEL_16x16, 0, 0, 0 );
    if( CHROMA_FORMAT )
    {
        int chroma_size = h->luma2chroma_pixel[PIXEL_16x16];
        int chroma_ssd = ssd_8_plane( h, chroma_size, 1, 0, 0 ) + ssd_8_plane( h, chroma_size, 2, 0, 0 );
        i_ssd += ((uint64_t)chroma_ssd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
    }
    return i_ssd;
}

static int rd_8_cost_mb( x264_t *h, int i_lambda2 )
{
    ...
    else if( h->param.b_cabac )
    {
        ...
        COPY_CABAC;
        macroblock_8_size_cabac( h, &cabac_tmp );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 32768 ) >> 16;
    }
    else
    {
        macroblock_8_size_cavlc( h );
...
static uint64_t rd_8_cost_subpart( x264_t *h, int i_lambda2, int i4, int i_pixel )
{
    ...
    i_ssd = ssd_8_plane( h, i_pixel, 0, block_idx_x[i4]*4, block_idx_y[i4]*4 );
    if( CHROMA444 )
    {
        int chromassd = ssd_8_plane( h, i_pixel, 1, block_idx_x[i4]*4, block_idx_y[i4]*4 )
                      + ssd_8_plane( h, i_pixel, 2, block_idx_x[i4]*4, block_idx_y[i4]*4 );
    ...
    if( h->param.b_cabac )
    {
        x264_cabac_t cabac_tmp;
        COPY_CABAC;
        subpartition_8_size_cabac( h, &cabac_tmp, i4, i_pixel );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
        i_bits = subpartition_8_size_cavlc( h, i4, i_pixel );

    return (i_ssd<<8) + i_bits;
}

uint64_t x264_8_rd_cost_part( x264_t *h, int i_lambda2, int i4, int i_pixel )
{
    uint64_t i_ssd, i_bits;
    int i8 = i4 >> 2;
    ...
    {
        int i_cost = rd_8_cost_mb( h, i_lambda2 );
        return i_cost;
    }

    if( i_pixel > PIXEL_8x8 )
        return rd_8_cost_subpart( h, i_lambda2, i4, i_pixel );
    ...
    int ssd_x = 8*(i8&1);
    int ssd_y = 8*(i8>>1);
    i_ssd = ssd_8_plane( h, i_pixel, 0, ssd_x, ssd_y );
    if( CHROMA_FORMAT )
    {
        int chroma_size = h->luma2chroma_pixel[i_pixel];
        int chroma_ssd = ssd_8_plane( h, chroma_size, 1, ssd_x>>CHROMA_H_SHIFT, ssd_y>>CHROMA_V_SHIFT )
                       + ssd_8_plane( h, chroma_size, 2, ssd_x>>CHROMA_H_SHIFT, ssd_y>>CHROMA_V_SHIFT );
        i_ssd += ((uint64_t)chroma_ssd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
    }
    ...
    if( h->param.b_cabac )
    {
        x264_cabac_t cabac_tmp;
        COPY_CABAC;
        partition_8_size_cabac( h, &cabac_tmp, i8, i_pixel );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
        i_bits = (uint64_t)partition_8_size_cavlc( h, i8, i_pixel ) * i_lambda2;

    return (i_ssd<<8) + i_bits;
}

static uint64_t rd_8_cost_i8x8( x264_t *h, int i_lambda2, int i8, int i_mode, pixel edge[4][32] )
{
    uint64_t i_ssd, i_bits;
    ...
    i_ssd = ssd_8_plane( h, PIXEL_8x8, 0, (i8&1)*8, (i8>>1)*8 );
    if( CHROMA444 )
    {
        int chromassd = ssd_8_plane( h, PIXEL_8x8, 1, (i8&1)*8, (i8>>1)*8 )
                      + ssd_8_plane( h, PIXEL_8x8, 2, (i8&1)*8, (i8>>1)*8 );
        chromassd = ((uint64_t)chromassd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
        i_ssd += chromassd;
    }
    ...
    if( h->param.b_cabac )
    {
        x264_cabac_t cabac_tmp;
        COPY_CABAC;
        partition_8_i8x8_size_cabac( h, &cabac_tmp, i8, i_mode );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
        i_bits = (uint64_t)partition_8_i8x8_size_cavlc( h, i8, i_mode ) * i_lambda2;

    return (i_ssd<<8) + i_bits;
}

static uint64_t rd_8_cost_i4x4( x264_t *h, int i_lambda2, int i4, int i_mode )
{
    uint64_t i_ssd, i_bits;
    int plane_count = CHROMA444 ? 3 : 1;
    int i_qp = h->mb.i_qp;
    ...
    i_ssd = ssd_8_plane( h, PIXEL_4x4, 0, block_idx_x[i4]*4, block_idx_y[i4]*4 );
    if( CHROMA444 )
    {
        int chromassd = ssd_8_plane( h, PIXEL_4x4, 1, block_idx_x[i4]*4, block_idx_y[i4]*4 )
                      + ssd_8_plane( h, PIXEL_4x4, 2, block_idx_x[i4]*4, block_idx_y[i4]*4 );
        chromassd = ((uint64_t)chromassd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
        i_ssd += chromassd;
    }
    ...
    if( h->param.b_cabac )
    {
        x264_cabac_t cabac_tmp;
        COPY_CABAC;
        partition_8_i4x4_size_cabac( h, &cabac_tmp, i4, i_mode );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
        i_bits = (uint64_t)partition_8_i4x4_size_cavlc( h, i4, i_mode ) * i_lambda2;

    return (i_ssd<<8) + i_bits;
}

static uint64_t rd_8_cost_chroma( x264_t *h, int i_lambda2, int i_mode, int b_dct )
{
    uint64_t i_ssd, i_bits;
    ...
    i_ssd = ssd_8_plane( h, chromapix, 1, 0, 0 )
          + ssd_8_plane( h, chromapix, 2, 0, 0 );
    ...
    if( h->param.b_cabac )
    {
        x264_cabac_t cabac_tmp;
        COPY_CABAC;
        chroma_8_size_cabac( h, &cabac_tmp );
        i_bits = ( (uint64_t)cabac_tmp.f8_bits_encoded * i_lambda2 + 128 ) >> 8;
    }
    else
        i_bits = (uint64_t)chroma_8_size_cavlc( h ) * i_lambda2;
...
#define SET_LEVEL(ndst, nsrc, l) {\
    if( sizeof(trellis_8_level_t) == sizeof(uint32_t) )\
        M32( &level_tree[levels_used] ) = pack16to32( nsrc.level_idx, l );\
    else\
        level_tree[levels_used] = (trellis_8_level_t){ nsrc.level_idx, l };\
...
static NOINLINE
int trellis_8_dc_shortcut( int sign_coef, int quant_coef, int unquant_mf, int coef_weight, int lambda2, uint8_t *cabac_state, int cost_sig )
{
...
int trellis_8_coef( int j, int const_level, int abs_level, int prefix, int suffix_cost,
                  int node_ctx, int level1_ctx, int levelgt1_ctx, uint64_t ssd, int cost_siglast[3],
                  trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                  trellis_8_level_t *level_tree, int levels_used, int lambda2, uint8_t *level_state )
{
...
static NOINLINE
int trellis_8_coef0_0( uint64_t ssd0, trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                     trellis_8_level_t *level_tree, int levels_used )
{
...
static NOINLINE
int trellis_8_coef0_1( uint64_t ssd0, trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                     trellis_8_level_t *level_tree, int levels_used )
{
    for( int j = 1; j < 8; j++ )
...
#define COEF(const_level, ctx_hi, j, ...)\
    if( !j || (int64_t)nodes_prev[j].score >= 0 )\
        levels_used = trellis_8_coef( j, const_level, abs_level, prefix, suffix_cost, __VA_ARGS__,\
...
static NOINLINE
int trellis_8_coef1_0( uint64_t ssd0, uint64_t ssd1, int cost_siglast[3],
                     trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                     trellis_8_level_t *level_tree, int levels_used, int lambda2,
                     uint8_t *level_state )
{
...
static NOINLINE
int trellis_8_coef1_1( uint64_t ssd0, uint64_t ssd1, int cost_siglast[3],
                     trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                     trellis_8_level_t *level_tree, int levels_used, int lambda2,
                     uint8_t *level_state )
{
...
static NOINLINE
int trellis_8_coefn_0( int abs_level, uint64_t ssd0, uint64_t ssd1, int cost_siglast[3],
                     trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                     trellis_8_level_t *level_tree, int levels_used, int lambda2,
...
static NOINLINE
int trellis_8_coefn_1( int abs_level, uint64_t ssd0, uint64_t ssd1, int cost_siglast[3],
                     trellis_8_node_t *nodes_cur, trellis_8_node_t *nodes_prev,
                     trellis_8_level_t *level_tree, int levels_used, int lambda2,
                     uint8_t *level_state, int levelgt1_ctx )
{
...
static ALWAYS_INLINE
int quant_8_trellis_cabac( x264_t *h, dctcoef *dct,
                         ...
                         int b_chroma, int dc, int num_coefs, int idx )
{
    ...
    if( last_nnz == 0 && !dc )
    {
        ...
        dct[0] = trellis_8_dc_shortcut( orig_coefs[0], quant_coefs[0], unquant_mf[0], coef_weight2[0], lambda2, cabac_state, cost_sig );
        return !!dct[0];
    }
...
#define TRELLIS_ARGS unquant_mf, zigzag, lambda2, last_nnz, orig_coefs, quant_coefs, dct,\
    ...
    trellis_8_level_t level_tree[64*8*2];
    int levels_used = 1;
    /* init trellis */
    trellis_8_node_t nodes[2][8] = {0};
    trellis_8_node_t *nodes_cur = nodes[0];
    trellis_8_node_t *nodes_prev = nodes[1];
    trellis_8_node_t *bnode;
    for( int j = 1; j < 8; j++ )
    ...
    level_state[12] = cabac_state[0]; // packed subset for copying into trellis_8_node_t
...
\
        ...
        XCHG( trellis_8_node_t*, nodes_cur, nodes_prev );\
...
\
        ...
        case 1:\
            ...
            levels_used = trellis_8_coef0_##ctx_hi( ssd0[0]-ssd1[0], nodes_cur, nodes_prev, level_tree, levels_used );\
            levels_used = trellis_8_coef1_##ctx_hi( ssd0[1]-ssd1[0], ssd1[1]-ssd1[0], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2, level_state );\
            goto next##ctx_hi;\
        case 2:\
            levels_used = trellis_8_coef1_##ctx_hi( ssd0[0], ssd1[0], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2, level_state );\
            levels_used = trellis_8_coefn_##ctx_hi( q, ssd0[1], ssd1[1], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2, level_state, levelgt1_ctx );\
            goto next1;\
        default:\
            levels_used = trellis_8_coefn_##ctx_hi( q-1, ssd0[0], ssd1[0], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2, level_state, levelgt1_ctx );\
            levels_used = trellis_8_coefn_##ctx_hi( q, ssd0[1], ssd1[1], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2, level_state, levelgt1_ctx );\
            goto next1;\
        }\
        next##ctx_hi:;\
...
static ALWAYS_INLINE
int quant_8_trellis_cavlc( x264_t *h, dctcoef *dct,
                         ...
                         int b_chroma, int dc, int num_coefs, int idx, int b_8x8 )
{
    ...
    int nC = b_chroma && dc ? 3 + (num_coefs>>2)
                            : ct_8_index[x264_mb_predict_non_zero_code( h, !b_chroma && dc ? (idx - LUMA_DC)*16 : idx )];
    ...
    if( !coef_mask )
        ...
        cavlc_8_block_residual_internal( h, ctx_block_cat, coefs + b_ac, nC );
...
int x264_8_quant_luma_dc_trellis( x264_t *h, dctcoef *dct, int i_quant_cat, int i_qp, int ctx_block_cat, int b_intra, int idx )
{
    if( h->param.b_cabac )
        return quant_8_trellis_cabac( h, dct,
    ...
    return quant_8_trellis_cavlc( h, dct,
...
static const uint8_t zigzag_8_scan2x2[4] = { 0, 1, 2, 3 };
static const uint8_t zigzag_8_scan2x4[8] = { 0, 2, 1, 4, 6, 3, 5, 7 };

int x264_8_quant_chroma_dc_trellis( x264_t *h, dctcoef *dct, int i_qp, int b_intra, int idx )
{
    ...
    {
        zigzag = zigzag_8_scan2x4;
        num_coefs = 8;
    }
    else
    {
        zigzag = zigzag_8_scan2x2;
        num_coefs = 4;
    }
...
int x264_8_quant_4x4_trellis( x264_t *h, dctcoef *dct, int i_quant_cat,
...
int x264_8_quant_8x8_trellis( x264_t *h, dctcoef *dct, int i_quant_cat,
                            int i_qp, int ctx_block_cat, int b_intra, int b_chroma, int idx )
{
    if( h->param.b_cabac )
    {
    ...
    for( int i = 0; i < 4; i++ )
    {
        int nz = quant_8_trellis_cavlc( h, dct,
