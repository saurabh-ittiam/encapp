...
static const uint8_t delta_8_tfi_divisor[10] = { 0, 2, 1, 1, 2, 2, 3, 3, 4, 6 };

static int slicetype_8_frame_cost( x264_t *h, x264_8_mb_analysis_t *a,
                                 x264_frame_t **frames, int p0, int p1, int b );

#define x264_8_weights_analyse x264_template(weights_analyse)
void x264_8_weights_analyse( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead );

#if HAVE_OPENCL
#include "slicetype-cl.h"
#endif

static void lowres_8_context_init( x264_t *h, x264_8_mb_analysis_t *a )
{
    a->i_qp = X264_LOOKAHEAD_QP;
    a->i_lambda = x264_lambda_tab[ a->i_qp ];
    mb_8_analyse_load_costs( h, a );
...
static void weight_8_get_h264( int weight_nonh264, int offset, x264_weight_t *w )
{
    w->i_offset = offset;
    w->i_denom = 7;
...
static NOINLINE pixel *weight_8_cost_init_luma( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, pixel *dest )
{
...
static NOINLINE void weight_8_cost_init_chroma( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, pixel *dstu, pixel *dstv )
{
    int ref0_distance = fenc->i_frame - ref->i_frame - 1;
...
static NOINLINE pixel *weight_8_cost_init_chroma444( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, pixel *dst, int p )
{
    int ref0_distance = fenc->i_frame - ref->i_frame - 1;
...
static int weight_8_slice_header_cost( x264_t *h, x264_weight_t *w, int b_chroma )
{
...
static NOINLINE unsigned int weight_8_cost_luma( x264_t *h, x264_frame_t *fenc, pixel *src, x264_weight_t *w )
{
    unsigned int cost = 0;
    int i_stride = fenc->i_stride_lowres;
    int i_lines = fenc->i_lines_lowres;
    ...
    if( w )
    {
        ...
        cost += weight_8_slice_header_cost( h, w, 0 );
    }
    else
        for( int y = 0; y < i_lines; y += 8, pixoff = y*i_stride )
            for( int x = 0; x < i_width; x += 8, i_mb++, pixoff += 8 )
...
static NOINLINE unsigned int weight_8_cost_chroma( x264_t *h, x264_frame_t *fenc, pixel *ref, x264_weight_t *w )
{
    unsigned int cost = 0;
    int i_stride = fenc->i_stride[1];
    int i_lines = fenc->i_lines[1];
    ...
    if( w )
    {
        ...
        cost += weight_8_slice_header_cost( h, w, 1 );
    }
    else
        for( int y = 0; y < i_lines; y += height, pixoff = y*i_stride )
            for( int x = 0; x < i_width; x += 8, pixoff += 8 )
...
static NOINLINE unsigned int weight_8_cost_chroma444( x264_t *h, x264_frame_t *fenc, pixel *ref, x264_weight_t *w, int p )
{
    unsigned int cost = 0;
    int i_stride = fenc->i_stride[p];
    int i_lines = fenc->i_lines[p];
...
void x264_8_weights_analyse( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead )
{
    ...
    for( int plane = 0; plane < (CHROMA_FORMAT ? 3 : 1) && !( plane && ( !weights[0].weightfn || b_lookahead ) ); plane++ )
    {
        ...
        if( plane )
        {
            ...
            weight_8_get_h264( round( guess_scale[plane] * 128 ), 0, &weights[plane] );
        ...
        if( !plane )
        {
            ...
            {
                x264_8_mb_analysis_t a;
                lowres_8_context_init( h, &a );
                slicetype_8_frame_cost( h, &a, &fenc, 0, 0, 0 );
            }
            mcbuf = weight_8_cost_init_luma( h, fenc, ref, h->mb.p_weight_buf[0] );
            origscore = minscore = weight_8_cost_luma( h, fenc, mcbuf, NULL );
        }
        else
        {
            if( CHROMA444 )
            {
                mcbuf = weight_8_cost_init_chroma444( h, fenc, ref, h->mb.p_weight_buf[0], plane );
                origscore = minscore = weight_8_cost_chroma444( h, fenc, mcbuf, NULL, plane );
            }
            else
            {
                ...
                if( !chroma_initted++ )
                    weight_8_cost_init_chroma( h, fenc, ref, dstu, dstv );
                mcbuf = plane == 1 ? dstu : dstv;
                origscore = minscore = weight_8_cost_chroma( h, fenc, mcbuf, NULL );
            }
        }
        ...
        for( int i_scale = start_scale; i_scale <= end_scale; i_scale++ )
        {
            ...
            for( int i_off = start_offset; i_off <= end_offset; i_off++ )
            {
                ...
                {
                    if( CHROMA444 )
                        s = weight_8_cost_chroma444( h, fenc, mcbuf, &weights[plane], plane );
                    else
                        s = weight_8_cost_chroma( h, fenc, mcbuf, &weights[plane] );
                }
                else
                    s = weight_8_cost_luma( h, fenc, mcbuf, &weights[plane] );
    ...
    {
        //scale lowres in lookahead for slicetype_8_frame_cost
...
static void slicetype_8_mb_cost( x264_t *h, x264_8_mb_analysis_t *a,
                               x264_frame_t **frames, int p0, int p1, int b,
...
typedef struct
{
    ...
    int *output_inter;
    int *output_intra;
} x264_8_slicetype_slice_t;

static void slicetype_8_slice_cost( x264_8_slicetype_slice_t *s )
{
    x264_t *h = s->h;
    ...
    for( h->mb.i_mb_y = start_y; h->mb.i_mb_y >= end_y; h->mb.i_mb_y-- )
        for( h->mb.i_mb_x = start_x; h->mb.i_mb_x >= end_x; h->mb.i_mb_x-- )
            slicetype_8_mb_cost( h, s->a, s->frames, s->p0, s->p1, s->b, s->dist_scale_factor,
                                 ...
                                 x264_frame_t **frames, int p0, int p1, int b )
{
    ...
    else
    {
        ...
        if( do_search[0] )
        {
            ...
            {
                x264_emms();
                x264_8_weights_analyse( h, fenc, frames[p0], 1 );
                w = fenc->weight[0];
            }
...
#endif
        {
            ...
            {
                x264_8_slicetype_slice_t s[X264_LOOKAHEAD_THREAD_MAX];
                ...
                for( int i = 0; i < h->param.i_lookahead_threads; i++ )
                {
                    ...
                    s[i] = (x264_8_slicetype_slice_t){ t, a, frames, p0, p1, b, dist_scale_factor, do_search, w,
                    ...
                    x264_threadpool_run( h->lookaheadpool, (void*)slicetype_8_slice_cost, &s[i] );
                }
                for( int i = 0; i < h->param.i_lookahead_threads; i++ )
            ...
            else
            {
                ...
                x264_8_slicetype_slice_t s = (x264_8_slicetype_slice_t){ h, a, frames, p0, p1, b, dist_scale_factor, do_search, w,
                    output_inter[0], output_intra[0] };
                slicetype_8_slice_cost( &s );
            }
...
static int slicetype_8_frame_cost_recalculate( x264_t *h, x264_frame_t **frames, int p0, int p1, int b )
{
    int i_score = 0;
...
static void macroblock_8_tree_finish( x264_t *h, x264_frame_t *frame, float average_duration, int ref0_distance )
{
...
static void macroblock_8_tree_propagate( x264_t *h, x264_frame_t **frames, float average_duration, int p0, int p1, int b, int referenced )
{
    ...
    if( h->param.rc.i_vbv_buffer_size && h->param.rc.i_lookahead && referenced )
        macroblock_8_tree_finish( h, frames[b], average_duration, b == p1 ? b - p0 : 0 );
}

static void macroblock_8_tree( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, int num_frames, int b_intra )
{
    int idx = !b_intra;
    ...
    if( b_intra )
        slicetype_8_frame_cost( h, a, frames, 0, 0, 0 );
    ...
    while( i-- > idx )
    {
        ...
        if( cur_nonb < idx )
            break;
        slicetype_8_frame_cost( h, a, frames, cur_nonb, last_nonb, last_nonb );
        ...
        if( h->param.i_bframe_pyramid && bframes > 1 )
        {
            ...
            slicetype_8_frame_cost( h, a, frames, cur_nonb, last_nonb, middle );
            ...
            while( i > cur_nonb )
            {
                ...
                if( i != middle )
                {
                    slicetype_8_frame_cost( h, a, frames, p0, p1, i );
                    macroblock_8_tree_propagate( h, frames, average_duration, p0, p1, i, 0 );
                }
                i--;
            }
            macroblock_8_tree_propagate( h, frames, average_duration, cur_nonb, last_nonb, middle, 1 );
        }
        else
        {
            while( i > cur_nonb )
            {
                slicetype_8_frame_cost( h, a, frames, cur_nonb, last_nonb, i );
                macroblock_8_tree_propagate( h, frames, average_duration, cur_nonb, last_nonb, i, 0 );
                i--;
            }
        }
        macroblock_8_tree_propagate( h, frames, average_duration, cur_nonb, last_nonb, last_nonb, 1 );
        last_nonb = cur_nonb;
    }

    if( !h->param.rc.i_lookahead )
    {
        slicetype_8_frame_cost( h, a, frames, 0, last_nonb, last_nonb );
        macroblock_8_tree_propagate( h, frames, average_duration, 0, last_nonb, last_nonb, 1 );
        XCHG( uint16_t*, frames[last_nonb]->i_propagate_cost, frames[0]->i_propagate_cost );
    }

    macroblock_8_tree_finish( h, frames[last_nonb], average_duration, last_nonb );
    if( h->param.i_bframe_pyramid && bframes > 1 && !h->param.rc.i_vbv_buffer_size )
        macroblock_8_tree_finish( h, frames[last_nonb+(bframes+1)/2], average_duration, 0 );
}

static int vbv_8_frame_cost( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int b )
{
    int cost = slicetype_8_frame_cost( h, a, frames, p0, p1, b );
    if( h->param.rc.i_aq_mode )
    {
        if( h->param.rc.b_mb_tree )
            return slicetype_8_frame_cost_recalculate( h, frames, p0, p1, b );
        else
...
static void calculate_8_durations( x264_t *h, x264_frame_t *cur_frame, x264_frame_t *prev_frame, int64_t *i_cpb_delay, int64_t *i_coded_fields )
{
...
static void vbv_8_lookahead( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, int num_frames, int keyframe )
{
    ...
    while( cur_nonb < num_frames )
    {
        ...
        if( next_nonb != cur_nonb )
        {
            ...
            frames[next_nonb]->i_planned_satd[idx] = vbv_8_frame_cost( h, a, frames, p0, cur_nonb, cur_nonb );
            ...
            calculate_8_durations( h, frames[cur_nonb], prev_frame, &h->i_cpb_delay_lookahead, &h->i_coded_fields_lookahead );
            if( prev_frame )
            {
        ...
        {
            frames[next_nonb]->i_planned_satd[idx] = vbv_8_frame_cost( h, a, frames, last_nonb, cur_nonb, i );
            ...
            calculate_8_durations( h, frames[i], prev_frame, &h->i_cpb_delay_lookahead, &h->i_coded_fields_lookahead );
            if( prev_frame )
            {
                frames[next_nonb]->f_planned_cpb_duration[prev_frame_idx] = (double)prev_frame->i_cpb_duration *
                                                                            h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
...
static uint64_t slicetype_8_path_cost( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, char *path, uint64_t threshold )
{
    uint64_t cost = 0;
    int loc = 1;
    int cur_nonb = 0;
    ...
    while( path[loc] )
    {
        ...
        if( path[next_nonb] == 'P' )
            cost += slicetype_8_frame_cost( h, a, frames, cur_nonb, next_nonb, next_nonb );
        else /* I-frame */
            cost += slicetype_8_frame_cost( h, a, frames, next_nonb, next_nonb, next_nonb );
        ...
        if( h->param.i_bframe_pyramid && next_nonb - cur_nonb > 2 )
        {
            ...
            cost += slicetype_8_frame_cost( h, a, frames, cur_nonb, next_nonb, middle );
            for( int next_b = loc; next_b < middle && cost < threshold; next_b++ )
                cost += slicetype_8_frame_cost( h, a, frames, cur_nonb, middle, next_b );
            for( int next_b = middle+1; next_b < next_nonb && cost < threshold; next_b++ )
                cost += slicetype_8_frame_cost( h, a, frames, middle, next_nonb, next_b );
        }
        else
            for( int next_b = loc; next_b < next_nonb && cost < threshold; next_b++ )
                cost += slicetype_8_frame_cost( h, a, frames, cur_nonb, next_nonb, next_b );
...
/* Uses strings due to the fact that the speed of the control functions is
   negligible compared to the cost of running slicetype_8_frame_cost, and because
   it makes debugging easier. */
static void slicetype_8_path( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, int length, char (*best_paths)[X264_LOOKAHEAD_MAX+1] )
{
    ...
    for( int path = 0; path < num_paths; path++ )
    {
        ...
        if( possible || !best_possible )
        {
            ...
            uint64_t cost = slicetype_8_path_cost( h, a, frames, paths[idx], best_cost );
            if( cost < best_cost )
            {
                best_cost = cost;
...
static int scenecut_8_internal( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int real_scenecut )
{
    ...
    slicetype_8_frame_cost( h, a, frames, p0, p1, p1 );
...
static int scenecut_8( x264_t *h, x264_8_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int real_scenecut, int num_frames, int i_max_search )
{
    ...
    if( real_scenecut && h->param.i_bframe )
    {
        ...
        for( int curp1 = p1; curp1 <= maxp1; curp1++ )
            if( !scenecut_8_internal( h, a, frames, p0, curp1, 0 ) )
        ...
        for( int curp0 = p0; curp0 <= maxp1; curp0++ )
            if( origmaxp1 > i_max_search || (curp0 < maxp1 && scenecut_8_internal( h, a, frames, curp0, maxp1, 0 )) )
    ...
    if( !frames[p1]->b_scenecut )
        return 0;
    return scenecut_8_internal( h, a, frames, p0, p1, real_scenecut );
}
...
void x264_8_slicetype_analyse( x264_t *h, int intra_minigop )
{
    x264_8_mb_analysis_t a;
    x264_frame_t *frames[X264_LOOKAHEAD_MAX+3] = { NULL, };
    int num_frames, orig_num_frames, keyint_limit, framecnt;
    int i_max_search = X264_MIN( h->lookahead->next.i_size, X264_LOOKAHEAD_MAX );
    int b_vbv_8_lookahead = h->param.rc.i_vbv_buffer_size && h->param.rc.i_lookahead;
    ...
    {
        if( h->param.rc.b_mb_tree )
            macroblock_8_tree( h, &a, frames, 0, keyframe );
        return;
    }
     ...
     * more RD-optimal. */
    if( (h->param.analyse.b_psy && h->param.rc.b_mb_tree) || b_vbv_8_lookahead )
        num_frames = framecnt;
    ...
    if( IS_X264_TYPE_AUTO_OR_I( frames[1]->i_type ) &&
        h->param.i_scenecut_threshold && scenecut_8( h, &a, frames, 0, 1, 1, orig_num_frames, i_max_search ) )
    {
    ...
    if( h->param.i_bframe )
    {
        if( h->param.i_bframe_adaptive == X264_B_ADAPT_TRELLIS )
        {
            if( num_frames > 1 )
            {
                ...
                for( int j = 2; j <= num_frames; j++ )
                    slicetype_8_path( h, &a, frames, j, best_paths );
        ...
        else if( h->param.i_bframe_adaptive == X264_B_ADAPT_FAST )
        {
            ...
            for( int j = 1; j < num_frames; j++ )
            {
                ...
                uint64_t cost_p = slicetype_8_path_cost( h, &a, frames+last_nonb, path, COST_MAX64 );
                strcpy( path+bframes, "BP" );
                uint64_t cost_b = slicetype_8_path_cost( h, &a, frames+last_nonb, path, cost_p );
        ...
        for( int j = 1; j < num_bframes+1; j++ )
        {
            if( frames[j]->i_forced_type == X264_TYPE_AUTO && IS_X264_TYPE_AUTO_OR_I( frames[j+1]->i_forced_type ) &&
                h->param.i_scenecut_threshold && scenecut_8( h, &a, frames, j, j+1, 0, orig_num_frames, i_max_search ) )
            {
                frames[j]->i_type = X264_TYPE_P;
    ...
    if( h->param.rc.b_mb_tree )
        macroblock_8_tree( h, &a, frames, X264_MIN(num_frames, h->param.i_keyint_max), keyframe );
    ...
    if( b_vbv_8_lookahead )
        vbv_8_lookahead( h, &a, frames, num_frames, keyframe );
...
void x264_8_slicetype_decide( x264_t *h )
{
    ...
    for( int i = 0; i < h->lookahead->next.i_size; i++ )
    {
        if( h->param.b_vfr_input )
        {
            ...
            h->lookahead->next.list[i]->i_duration = delta_8_tfi_divisor[h->lookahead->next.list[i]->i_pic_struct];
    ...
    if( h->param.rc.i_rc_method != X264_RC_CQP )
    {
        ...
        slicetype_8_frame_cost( h, &a, frames, p0, p1, b );

        if( (p0 != p1 || bframes) && h->param.rc.i_vbv_buffer_size )
        {
            /* We need the intra costs for row SATDs. */
            slicetype_8_frame_cost( h, &a, frames, b, b, b );
    ...
    {
        x264_emms();
        x264_8_weights_analyse( h, h->lookahead->next.list[bframes], h->lookahead->last_nonb, 0 );
    }
    ...
    for( int i = 0; i <= bframes; i++ )
    {
        ...
        if( i )
        {
            calculate_8_durations( h, h->lookahead->next.list[i], h->lookahead->next.list[i-1], &h->i_cpb_delay, &h->i_coded_fields );
            h->lookahead->next.list[0]->f_planned_cpb_duration[i-1] = (double)h->lookahead->next.list[i]->i_cpb_duration *
                                                                      h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
        }
        else
            calculate_8_durations( h, h->lookahead->next.list[i], NULL, &h->i_cpb_delay, &h->i_coded_fields );
    }
}

int x264_8_rc_analyse_slice( x264_t *h )
{
    int p0 = 0, p1, b;
    int cost;
    x264_emms();
    ...
    {
        cost = slicetype_8_frame_cost_recalculate( h, frames, p0, p1, b );
        if( b && h->param.rc.i_vbv_buffer_size )
            slicetype_8_frame_cost_recalculate( h, frames, b, b, b );
    }
